...
#define NULLSQ -1
#define initboard Board(startpos,NULLSQ)
...
bool isprom(std::string board, int s, int d) {if ((board[s] == WP && d > 55) || (board[s] == BP && d < 8)){return 1;} else {return 0;}}
int ucitoint(std::string move) {return (cnumtonum(move[1])*8+letttonum(move[0]))*64 + (cnumtonum(move[3])*8+letttonum(move[2]));}
std::string intosq(int8_t sq) {
	std::string r = "";
	r += numtolett(sq);
	r += numtocnum(sq);
	return r;
...
class Board {
	...
	void push(uint16_t move, char prompiece=Chess::defaultprom) {
		...
		stack.wkc[stack.len] = wkc;
		stack.bkc[stack.len] = bkc;
		stack.wqc[stack.len] = wqc;
		stack.bqc[stack.len] = bqc;
		stack.len++;
		zobrist ^= TTables::color;
	...
	void pop() {
		...
		wkc = stack.wkc[stack.len];
		bkc = stack.bkc[stack.len];
		wqc = stack.wqc[stack.len];
		bqc = stack.bqc[stack.len];
		zobrist ^= TTables::color;
	...
	bool is_legal(Move move) {
		uint8_t kl = locate_king();
		bool bad = 0;
		if (is_castling_move(move)) {
			change_turn();
			if (can_capture_on_square(kl)) {bad = 1;}
			if (move == WKC) {
				if (can_capture_on_square(1) || can_capture_on_square(2)) {bad = 1;}
			} else if (move == WQC) {
				if (can_capture_on_square(4) || can_capture_on_square(5) || can_capture_on_square(6)) {bad = 1;}
			} else if (move == BKC) {
				if (can_capture_on_square(57) || can_capture_on_square(58)) {bad = 1;}
			} else if (move == BQC) {
				if (can_capture_on_square(60) || can_capture_on_square(61) || can_capture_on_square(62)) {bad = 1;}
			}
			change_turn();
		}
		if (bad) {return 0;}
	...
	void set_from_fen_command(strstack splitcm) {
	...
	void set_from_fen_string(std::string fen) {
	...
	std::string string_id() {
		std::string brd = "";
		for (uint8_t i=0;i<64;i++) {
			brd += piece_to_gui[board[i]];
		}
		std::string c = (turn?"w":"b");
		brd += c;
		return brd;
	}
};
...
void print_moveinfo(SearchResult lmcout, Board b, Time totime, Value alpha=-Tuning::INF, Value beta=Tuning::INF) {
	...
	std::string nps = std::to_string(lmcout.nps);
	...
	if (lmcout.cp+lmcout.depth+1 >= Tuning::LOWER_MATE) {
		...
		int8_t ply = Tuning::LOWER_MATE+lmcout.cp;
		score = "mate -"+std::to_string((ply+1)/2);
	} else {
